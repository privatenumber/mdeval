...
describe('--watch', () => {
	test('re-renders on .md change', async () => {
	...
	test('-w short alias works', async () => {
		await using fixture = await createFixture({
			'data.md': '<!--mdeval\nlet v = 1;\nexport { v };\n-->\n\n<!--mdeval v-->0<!--/mdeval-->\n',
		});
		const dataPath = fixture.getPath('data.md');
	...
	test('--watch=true form works', async () => {
		await using fixture = await createFixture({
			'data.md': '<!--mdeval\nlet v = 1;\nexport { v };\n-->\n\n<!--mdeval v-->0<!--/mdeval-->\n',
		});
		const dataPath = fixture.getPath('data.md');
	...
	test('re-renders when an imported .ts file changes', async () => {
		await using fixture = await createFixture({
	...
	test('re-renders when an imported .json file changes', async () => {
		await using fixture = await createFixture({
			'helper.json': '{"value": 1}\n',
			'data.md': [
				'<!--mdeval',
	...
	test('re-renders any of multiple files', async () => {
		await using fixture = await createFixture({
	...
	test('survives a runtime error in a script block', async () => {
		await using fixture = await createFixture({
	...
	test('with zero matches exits 1 before entering watch mode', async () => {
		await using fixture = await createFixture({});
	...
	test('--help shows the watch flag', async () => {
	...
	test('without --watch, the CLI does one render and exits', async () => {
		await using fixture = await createFixture({
			'data.md': '<!--mdeval\nexport const v = 1;\n-->\n\n<!--mdeval v-->0<!--/mdeval-->\n',
		});
		await mdeval(
